#!/usr/bin/env bash
# Real Day-0 Kaggle/T4 run: four SSL ablations, three label fractions, and OOD.
set -euo pipefail
...
OUT="${DAY0_OUT:-outputs/day0}"
BACKBONE="${DAY0_BACKBONE:-resnet18}"
EPOCHS="${DAY0_EPOCHS:-40}"
BATCH_SIZE="${DAY0_BATCH_SIZE:-32}"
NUM_WORKERS="${DAY0_NUM_WORKERS:-2}"
# Empty means full ChestMNIST train. Set DAY0_SUBSET=20000 only if needed for wall-clock.
SUBSET="${DAY0_SUBSET:-}"
SEED="${DAY0_SEED:-42}"
...
archive = Path(shutil.make_archive("day0_outputs", "zip", root_dir=out))
