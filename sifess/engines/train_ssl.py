...
def _merge_cfg(args) -> Dict[str, Any]:
    ...
    cfg.setdefault("lambda_eq", 0.5)
    cfg.setdefault("lambda_orth", 0.01)
...
def train(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # SO(2)-on-2D: can set lambda_orth=0 via config
    if cfg.get("so2_only", False):
        lambda_orth = 0.0
    ...
    for epoch in range(int(cfg["epochs"])):
        model.train()
        running = {"loss": 0.0, "l_dino": 0.0, "l_eq": 0.0, "n": 0}
